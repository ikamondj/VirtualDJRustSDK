...
struct IVdjOAuth
{
    ...
    // open will add response_type=, redirect_uri= and state= to the authorizeUrl.
    // anything else should be added beforehand to the url.

    virtual void VDJ_API getToken(
        const char* code,
        const char* tokenUrl,
        const char* tokenPost = 0
    ) = 0;
    // getToken will POST code=, grant_type= and redirect_uri.
    // anything else should be added in tokenPost.

    virtual void VDJ_API refreshToken(
        const char* refresh_token,
        const char* tokenUrl,
        const char* tokenPost = 0
    ) = 0;
    // refreshToken will POST code= and grant_type.
    // anything else should be added in tokenPost.
};
...
public:
    ...
    virtual HRESULT VDJ_API IsLogged()
    {
        return E_NOTIMPL;
    }
    // return S_OK if the user is logged in, S_FALSE if he's not,
    // or E_NOTIMPL if you don't need virtualdj to handle login

    virtual HRESULT VDJ_API OnLogin()
    {
        return E_NOTIMPL;
    }

    virtual HRESULT VDJ_API OnLogout()
    {
        return E_NOTIMPL;
    }
    ...
    // when the user type something in the search field, this function will be called.
    // call tracksList->add(...) for each tracks from your search results.

    virtual HRESULT VDJ_API OnSearch(
        const char* search,
        IVdjTracksList* tracksList
    ) = 0;

    // your OnSearch function can take a long time to return.
    // If the user cancel or do a new search in the meantime,
    // we will call this function so you can unblock and return faster.

    virtual HRESULT VDJ_API OnSearchCancel()
    {
        return E_NOTIMPL;
    }

    // if you didn't populate the streamUrl fields in tracksList,
    // this function will be called per-track when the user loads a track onto a deck

    virtual HRESULT VDJ_API GetStreamUrl(
        const char* uniqueId,
        IVdjString& url,
        IVdjString& errorMessage
    )
    {
        return E_NOTIMPL;
    }
    ...
    virtual HRESULT VDJ_API GetFolderList(
        IVdjSubfoldersList* subfoldersList
    )
    {
        return E_NOTIMPL;
    }
    ...
    virtual HRESULT VDJ_API GetFolder(
        const char* folderUniqueId,
        IVdjTracksList* tracksList
    )
    {
    ...
    // if you want to implement extended behavior (like offline cache, buy-links, etc),
    // you can add items to the context menu of your tracks or your folders

    virtual HRESULT VDJ_API GetContextMenu(
        const char* uniqueId,
        IVdjContextMenu* contextMenu
    )
    {
        return E_NOTIMPL;
    }

    virtual HRESULT VDJ_API OnContextMenu(
        const char* uniqueId,
        size_t menuIndex
    )
    {
    ...
    virtual HRESULT VDJ_API GetFolderContextMenu(
        const char* folderUniqueId,
        IVdjContextMenu* contextMenu
    )
    {
    ...
    virtual HRESULT VDJ_API OnFolderContextMenu(
        const char* folderUniqueId,
        size_t menuIndex
    )
    {
...
static const GUID IID_IVdjPluginOnlineSource =
{
    ...
    0x0ccf,
    0x4cab,
    { 0xaa, 0x50, 0x1c, 0x04, 0xea, 0xb6, 0xb8, 0x5d }
};
